...
- **Имя.** Корневой тип `AppConfig`; секции `dataset`, `sources`, `segmentation`, `text_dynamics`, `preprocess`, `cache`, `model`, `training`, `retrieval`, `runtime`, `output`. Registry kinds: `source`, `ocr`, `segmenter`, `text_dynamics`, `filter`, `encoder`, `fusion`, `graph_encoder`, `dataset`, `miner`, `retriever`, `reranker`, `metric`. Встроенные имена: `precomputed_paddleocr`, `external_json`, `confidence`, `multilingual_e5`, `text_nodes`, `gat`, `v4rl`, `hard_negative`, `faiss_cosine`, `recall_at_k`.
- **Форма переключаемой стадии.** Секции `segmentation` и `text_dynamics` имеют тип `ComponentConfig`: `backend` (имя из реестра либо `null` — стадия выключена), `weights` (логическое имя записи манифеста весов, не путь) и `params` (произвольный mapping, который разбирает сама реализация). Параметры намеренно не типизированы в `AppConfig`: они принадлежат реализации, и это не даёт общим секциям обрастать полем на каждую добавленную модель.
...
- **Выход.** `FrameText` (см. `tgseqloc.data.formats`): кортеж `TextDetection` с нормализованным `box` в `[0,1]`, строкой `text`, `confidence` и `dynamic_score` (`None`, пока стадия динамики не отработала), плюс `image_size` в пикселях и `model_identity`. Свойства `.boxes` и `.texts` дают выровненные списки для fusion. Раньше возвращалась пара `(boxes, texts)`, терявшая confidence и не оставлявшая места атрибуту динамики.
