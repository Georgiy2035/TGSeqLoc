...
_unused = dict({"без текста": "f256-notext-f{f}-s{s}", "слагаемое": "f256-add-f{f}-s{s}",
...
ctx = robotcar({"без текста": "f256-notext-f{f}-s{s}", "слагаемое": "f256-add-f{f}-s{s}",
                "набор строк": "f256-set-f{f}-s{s}", "перемешанный набор": "f256-shufset-f{f}-s{s}",
                "набор + нулевой старт": "f256-set0-f{f}-s{s}", "перемеш. набор + нулевой": "f256-shufset0-f{f}-s{s}"})
if ctx:
    compare(ctx, [("набор + нулевой старт", "без текста"), ("набор + нулевой старт", "набор строк"),
                  ("набор + нулевой старт", "слагаемое"), ("набор + нулевой старт", "перемеш. набор + нулевой"),
                  ("перемеш. набор + нулевой", "без текста")])

PATHS = {"notext": str(D / "v256_runs/notext_{s}.json"), "node": str(D / "v256_runs/node_{s}.json"),
         "set": str(D / "v256_runs/set_{s}.json"), "set0": str(D / "zero_runs/v4rl_set0_{s}.json")}
def metrics(p):
    try:
...
V = {a: {s: m for s in range(42, 52) if (m := metrics(PATHS[a].format(s=s)))} for a in PATHS}
names = {"notext": "без текста", "node": "узел", "set": "набор строк", "set0": "набор + нулевой старт"}
for a, r in V.items():
    if r:
...
for x, y in (("set0", "set"), ("set0", "node"), ("set0", "notext"), ("set", "node")):
